...
export class TagDetailComponent {
  ...
  async initType(){
    switch (this.type) {
      ...
      case "AI":
        ...
        this.numberOfAlarms = this.tag.alarms.length;
        this.ready=true;
        break;
      case "AO":
        this.ao=true;
  ...
  displayAlarms() {
    if (this.ai){
      this.router.navigate(['/AiAlarms', this.tag.id]);
    }
  }

  deleteTag() {
    if(this.ai){
      this.tagService.deleteAiTag(this.tag.id).subscribe(
        response => {
          console.log("Deleted!");
      ...
      this.tagService.deleteDiTag(this.tag.id).subscribe(
        response => {
          console.log("Deleted!");
          this.router.navigate(['tagDisplay'])
        }
      ...
      this.tagService.deleteDoTag(this.tag.id).subscribe(
        response => {
          console.log("Deleted!");
          this.router.navigate(['tagDisplay'])
        }
      ...
      this.tagService.deleteAoTag(this.tag.id).subscribe(
        response => {
          console.log("Deleted!");
          this.router.navigate(['tagDisplay'])
        }
